...
class Wallet {
  ...
  createAccount() {
    let isZero = (this.accounts.length == 0);
    ...
    const new_account = { address, publicKey, privateKey, balance: 0 };
    walletUtils.saveWalletPrivateKey({ address, privateKey, isZero })
    this.accounts.push({...new_account, isZero});
    return {address, balance: STARTING_BALANCE};
  }

  // TODO: create transaction with add-on
  createTransaction({ chain, sender, recipient, amount, addon, gas }) {
    ...
    if(senderWallet) {
      ...
      return new Transaction({ senderWallet, recipient, amount, addon: addon || null, gas })
    }
  }
